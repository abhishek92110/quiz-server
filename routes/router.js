...
const saveQuiz  = require("../models/SaveQuiz")
const verifyUser = require("../middleware/verifyUser")
// const allCourse  = require("../models/AllCourse")
...
router.post('/user-login', async (req, res) => {
    try {
        ...
        if (existingUser) 
            {
                console.log("if condition of user login")
            // Generate a token with the user's ID
            const token = jwt.sign({ userId: existingUser._id, userName:existingUser.name, userEmail:existingUser.email, course:existingUser.course }, jwt_secret);

            // Email already exists, return status true and token
            return res.status(200).json({ status: true, token, name:existingUser.name, course:existingUser.course });
        }
    ...
    try
     {
        ...
        await saveQuiz.updateOne({user:req.userId, category:data.category,date:data.date},{$set:quizData},{upsert:true})
...
router.get('/active-quiz',async(req,res)=>{
    ...
    activeQuiz.map(data=>{
        ...
        subCourseData[0].subCourse.map(element=>{
    ...
    try
     {
        ...
        await saveQuiz.updateOne({user:data.id, category:data.category,date:data.examDate},{$set:{question:data.question, status:true, marks:data.marks}},{upsert:true})
    ...
    try
     {
        ...
        // console.log("full date =",fullDate,userAnswer,category,req.userId)

        res.status(201).json({ status: true, userAnswer:userAnswer });
    } 
    catch (error) 
    {
        console.error('Error getting save quiz question:', error);
        res.status(500).json({ status: false, error: 'Server error' });
    }
});
    ...
    try
     {
        ...
        console.log("user id and category =",category, userId, date, req.header("examDate"))

        let userAnswer = await saveQuiz.find({category:category,user:userId, date:date})
...
router.delete("/deleteStudent",async(req,res)=>{
    ...
    // data = data.json()
    // // console.log("registrationNo delete student",data)
    ...
    const data = JSON.parse(dataHeader);

    const { RegistrationNo, Course, month } = data;

    
    let length = RegistrationNo.split('/')[2].split('-')[1];

    // console.log("data =", data);

    console.log("details =", RegistrationNo, Course, length, month);

    // console.log("details =",registrationNo,course)


    let deleteStudent = await registerStudentDev.deleteOne({RegistrationNo:RegistrationNo})

    let count = await totalRegistration.countDocuments({ Course:Course, RegistrationNo, month: month });
